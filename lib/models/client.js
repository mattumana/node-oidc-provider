...
module.exports = function getClient(provider) {
  ...
  function getAdapter() {
    if (!adapter) adapter = new (instance(provider).configuration('adapter'))('Client');
    return adapter;
  }
  ...
  class Client {
    ...
    backchannelLogout(sub, sid) {
      ...
      // EE-added
      // ===============================================
      provider.Account.findById.call(this, sub)
        .then(account => account.claims())
        .then((claims) => {
          logoutToken.set('email', claims.email);
          return logoutToken.sign(this, { noExp: true });
        })
        .then(token => got.post(
          this.backchannelLogoutUri,
          provider.httpOptions({ body: { logout_token: token } }))
        )
    ...
    static find(id, opts) {
      if (cache.has(id)) {
        ...
        if (found.noManage || (!opts || !opts.fresh)) {
